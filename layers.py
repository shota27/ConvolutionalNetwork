...
class Convolution:
    ...
    def forward(self, x):
        ...
        畳み込み層を通過する時にかけられる重みWの形状で計算後の特徴量次元数が決まる
        """
        FN, C, FH, FW = self.W.shape
        N, C, H, W = x.shape  #入力のチャンネル数はCに保存
...
class GlobalAveragePooling:
    ...
    def forward(self, x):
        ...
        #        N, C, H, W = x_ndarray.shape
        #        self.x_shape=x_ndarray.shape
        self.x_shape=x.shape
        
        #各チャンネルの平均を計算、順に並べる
        
        return  np.mean(x,axis=(2,3)) #N,C 第２次元に各チャンネルの平均値  axis2,3に沿って平均
    ...
    def backward(self, dout):
        ...
        マックスプーリングでは、順伝播計算時に最大値となった場所だけに勾配を伝える
        順伝播計算時に最大値となった場所は、self.arg_maxに保持されている
        dout : 出力層側から伝わってきた勾配  N,C
        return : 入力層側へ伝える勾配   N,C,H,W
        """
        #各画素が出力に影響するので勾配は各画素に伝播する
        #各層の画素数で割らない場合、不自然に大きくなる　順伝播では、チャンネル内の画素の平均をとって1つの値になっているため

        N,C,H,W=self.x_shape
        dx=np.tile(dout[:,:,np.newaxis,np.newaxis],(1,1,H,W))/(H*W)
